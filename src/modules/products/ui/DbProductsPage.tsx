...
export function DbProductsPage({ queryClient }: DbProductsPageProps) {
  ...
  const [productCollection] = useState(() => createProductCollection(queryClient))
  const { data: products, isLoading, isError } = useLiveQuery(
    (q) => {
      const query = q
      ...
      return showLowStockOnly
        ? query.where(({ product }) => not(gt(product.stock, LOW_STOCK_THRESHOLD)))
        : query
    },
