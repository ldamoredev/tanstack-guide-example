  ...
  return (
    <main className="page-wrap px-3 py-8 sm:px-4 sm:py-12">
      <section className="island-shell rounded-[1.5rem] p-4 sm:rounded-[2rem] sm:p-8">
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          ...
          <p className="m-0 self-center text-sm text-[var(--sea-ink-soft)]">
            {copy.products.virtual.querySource(
      ...
      <section className="island-shell mt-6 rounded-2xl p-4 sm:mt-8 sm:p-5">
        <div className="hidden grid-cols-[minmax(0,2fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_auto] gap-4 border-b border-[var(--line)] pb-3 text-sm font-semibold text-[var(--sea-ink)] md:grid">
          ...
          >
            {rowVirtualizer.getVirtualItems().map((virtualRow) => {
              ...
              return (
                <div
                  key={row.virtualId}
                  className="grid grid-cols-[minmax(0,1fr)_auto] gap-x-3 gap-y-1 border-b border-[var(--line)] px-4 py-3 text-sm text-[var(--sea-ink-soft)] odd:bg-[var(--surface-strong)] even:bg-[color-mix(in_oklab,var(--surface)_95%,var(--lagoon)_5%)] md:grid-cols-[minmax(0,2fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_auto] md:gap-4 md:py-4"
                  style={{
                    position: 'absolute',
                    top: 0,
                    left: 0,
                ...
                >
                  ...
                  <span className="text-xs md:text-sm">{row.sku}</span>
                  <span className="hidden md:inline">
                    {getLocalizedCategoryLabel(row.categoryId, locale)}
                  </span>
                  <span className="hidden md:inline">
                    {formatUsdCurrency(row.price, locale)}
                  </span>
