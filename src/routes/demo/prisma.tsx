...
const createTodo = createServerFn({
  ...
  .handler(async ({ data }) => {
    if (!isPrismaDemoReady || !prisma) {
      throw new Error(PRISMA_SETUP_MESSAGE)
    }
...
function DemoPrisma() {
  ...
  const { isConfigured, todos } = Route.useLoaderData()
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!isConfigured && (
          <div
            className="mb-4 rounded-lg border px-4 py-3 text-sm text-indigo-200"
            style={{
              background: 'rgba(93, 103, 227, 0.08)',
              borderColor: 'rgba(93, 103, 227, 0.25)',
            }}
          >
            {PRISMA_SETUP_MESSAGE}
          </div>
        )}
